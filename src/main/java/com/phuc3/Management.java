...
import com.phuc3.controller.TaskController;
import com.phuc3.controller.TypeController;
...
public class Management {
    private final static Logger logger = Logger.getLogger(Management.class);
    ...
    public boolean addType(String name) throws Exception {
        ...
        if (name.isEmpty()) {
            logWarn("Ten danh muc khong duoc de trong");
        }
        if (!isMatchingRegex(name, rawPattern)) {
            logWarn("Ten danh muc phai chua chu cai A – Z");
        }
        if (isTypeDuplicated(name)) {
            logWarn("Ten danh muc da ton tai");
        }
        Type type = new Type(name);
        if (TypeController.addType(type)) {
            logInfo("Da them thanh cong: " + type);
            refreshTypes();
            return true;
        }
        return false;
    ...
    public boolean deleteType(String id) throws Exception {
        ...
        if (type == null) {
            logWarn("Khong tim thay danh muc");
        }
        if (TypeController.deleteType(idNum)) {
            logInfo("Da xoa thanh cong: " + type);
            refreshTypes();
            return true;
        }
        return false;
    ...
    public boolean addTask(String requirementName, String assignee, String reviewer, String typeID, String date, String planFrom, String planTo) throws Exception {
        ...
        if (!isMatchingRegex(requirementName, rawPattern) || !isMatchingRegex(assignee, rawPattern) || !isMatchingRegex(reviewer, rawPattern)) {
            logWarn("Ten cong viec hoac ten nguoi phai chua chu cai A – Z");
        }
        int taskTypeIDNum;
        try {
        ...
        if (TaskController.addTask(task)) {
            logInfo("Da them thanh cong: " + task);
            refreshTasks();
            return true;
        }
        return false;
    ...
    public boolean deleteTask(String id) throws Exception {
        ...
        if (TaskController.deleteTask(idNum)) {
            logInfo("Da xoa thanh cong: " + task);
            refreshTasks();
            return true;
        }
        return false;
